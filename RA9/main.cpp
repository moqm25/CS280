...
int main(int argc, char *argv []) {
    ...
    bool result=true , newcust=false, rejcust = false, invcust = false, avedata =
    false;
    if( argc >= 2 ) {
        file.open(argv[1]);
        if( !file){
    ...
    if(invcust)
    {
        ...
        cout << endl;
        cout << "List of invalid electricity customers records:" << endl <<
        endl;
    ...
    if(avedata){
        ...
        for ( int i = 0; i < UtilityList.size(); i++ ){
            ...
            if( account >= 0 && account <= 99999){
                ...
                ElectricityCustomer * elecPtr = (ElectricityCustomer *)
                UtilityList[i];
        ...
        //cout << countElec << endl;
        if(countElec != 0){
            cout << "Average Electricity customers Usage of KWattHours: " <<
            sumElec /countElec << endl;
